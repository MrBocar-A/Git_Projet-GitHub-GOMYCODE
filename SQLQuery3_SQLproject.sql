...
SELECT * FROM EMPLOYEES;
	...
	--'LPAD' n'est pas une option nom de fonction intégrée reconnue. on va utiliser la combinaison de commande REPLICATE & RIGHT 
...
SELECT 
	salary + Commission as full_salary,
	DATEDIFF(year,HIRE_DATE,'2024-11-05') as Seniority,
	EMPLOYEE_NUMBER,FIRST_NAME,LAST_NAME
FROM 
	EMPLOYEES
WHERE
	salary + Commission >= 8000 AND TITLE IN ('Dr.', 'Mr.')
...
WHERE 
    ...
    AND (SUBSTRING(PRODUCT_NAME, 3, 1) = 't' 
         OR SUBSTRING(PRODUCT_NAME, 3, 1) = 'T') 
...
SELECT * FROM SUPPLIERS,CUSTOMERS
...
SELECT SUPPLIER_NUMBER,COMPANY,ADDRESS,PHONE FROM SUPPLIERS
SELECT * FROM CATEGORIES
...
WHERE 
    CUSTOMERS.CITY = 'Berlin'AND CATEGORIES.CATEGORY_NAME = 'Desserts'
GROUP BY 
    CUSTOMERS.CUSTOMER_CODE
HAVING 
    COUNT(DISTINCT PRODUCTS.PRODUCT_REF) <= 1;
...
SELECT 
    CUSTOMERS.CUSTOMER_CODE AS CustomerNumber,
    CUSTOMERS.COMPANY AS CompanyName,
    CUSTOMERS.PHONE AS PhoneNumber,
    COALESCE(SUM(ORDERS.SHIPPING_COST), 0) AS TotalAmount,
    CUSTOMERS.COUNTRY
FROM 
    CUSTOMERS
LEFT JOIN 
    ...
    AND DATEPART(dw, ORDERS.ORDER_DATE) = 2 
WHERE 
...
SELECT 
    ...
    CUSTOMERS.COMPANY as CompanyName,
    CUSTOMERS.PHONE as TelephoneNumber
FROM 
    CUSTOMERS
JOIN 
    ORDERS ON CUSTOMERS.CUSTOMER_CODE = ORDERS.CUSTOMER_CODE
...
SELECT 
    CUSTOMERS.CUSTOMER_CODE as CustomerCode,
    COUNT(ORDERS.ORDER_NUMBER) as NumberOfOrders
FROM 
    CUSTOMERS
LEFT JOIN 
    ORDERS ON CUSTOMERS.CUSTOMER_CODE = ORDERS.CUSTOMER_CODE
...
SELECT 
    COUNT(CASE WHEN YEAR(ORDER_DATE) = 1996 THEN 1 END) AS OrdersIn1996,
    COUNT(CASE WHEN YEAR(ORDER_DATE) = 1997 THEN 1 END) AS OrdersIn1997,
    (COUNT(CASE WHEN YEAR(ORDER_DATE) = 1997 THEN 1 END) - COUNT(CASE WHEN YEAR(ORDER_DATE) = 1996 THEN 1 END)) AS Difference
FROM 
    ORDERS;
